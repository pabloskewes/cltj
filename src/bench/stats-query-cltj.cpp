...
void query(const std::string &file, const std::string &queries, const uint64_t limit, const uint64_t timeout) {
    ...
    if (result) {
        ...
        for (string &query_string: dummy_queries) {
            ...
            auto total_selects = graph.tries[0].count_select + graph.tries[1].count_select + graph.tries[2].count_select + graph.tries[3].count_select + graph.tries[4].count_select + graph.tries[5].count_select;
            cout << nQ << ";" << res.size() << ";" << time << ";" << total_selects << endl;
            nQ++;
...
int main(int argc, char *argv[]) {
    ...
    if (type == "normal") {
        query<cltj::compact_ltj_stats, ltj::util::trait_distinct>(index, queries, limit, timeout);
    } else if (type == "star") {
        query<cltj::compact_ltj_stats, ltj::util::trait_size>(index, queries, limit, timeout);
    } else {
