...
void query(
    ...
    if (result) {
        ...
        for (string& query_string : dummy_queries) {
            ...
            typedef ltj::ltj_iterator_metatrie_hash<index_scheme_type, uint8_t, uint64_t> iterator_type;
#if ADAPTIVE
            typedef ltj::ltj_algorithm_hash<iterator_type, ltj::veo::veo_adaptive<iterator_type, trait_type>>
                algorithm_type;

#else
            typedef ltj::ltj_algorithm_hash<iterator_type, ltj::veo::veo_simple<iterator_type, trait_type>>
                algorithm_type;
#endif
...
int main(int argc, char* argv[]) {
    ...
    if (type == "normal") {
        if (count_only)
            query<cltj::compact_ltj_metatrie_hash, ltj::util::trait_distinct, true>(
                index, queries, limit, timeout
            );
        else
            query<cltj::compact_ltj_metatrie_hash, ltj::util::trait_distinct>(index, queries, limit, timeout);
    } else if (type == "star") {
        if (count_only)
            query<cltj::compact_ltj_metatrie_hash, ltj::util::trait_size, true>(
                index, queries, limit, timeout
            );
        else
            query<cltj::compact_ltj_metatrie_hash, ltj::util::trait_size>(index, queries, limit, timeout);
    } else {
