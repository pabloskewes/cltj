...
int main(int argc, char **argv) {
  if (argc != 7) {
    std::cout << argv[0]
              << " <index> <queries> <updates> <ratio> <limit> <type>"
              << std::endl;
    return 0;
  }
  ...
  ); // ratio of updates per query {0.001, 0.01, 0.1, 1, 10, 100, 1000}
  ...
  if (ratio < 1) {
    ...
    if (type == "normal") {
      query_qpu<cltj::compact_ltj_metatrie_dyn, ltj::util::trait_distinct>(
          index, qs, us, limit, qpu
      );
    } else if (type == "star") {
      query_qpu<cltj::compact_ltj_metatrie_dyn, ltj::util::trait_size>(
          index, qs, us, limit, qpu
      );
    } else {
      std::cout << "Type of index: " << type << " is not supported."
                << std::endl;
    }
  } else {
    auto upq = (uint64_t)ratio;
    if (type == "normal") {
      query_upq<cltj::compact_ltj_metatrie_dyn, ltj::util::trait_distinct>(
          index, qs, us, limit, upq
      );
    } else if (type == "star") {
      query_upq<cltj::compact_ltj_metatrie_dyn, ltj::util::trait_size>(
          index, qs, us, limit, upq
      );
    } else {
      std::cout << "Type of index: " << type << " is not supported."
