...
int main(int argc, char **argv) {
  ...
  for (uint64_t i = 0; i < num_build; ++i) {
    if (i % (num_build / 1000) == 0) {
      ...
      std::cout << "\r check build: " << per << "% (" << i << "/" << num_build
                << ")" << std::flush;
    }
  ...
  std::cout << "\r check build: 100% (" << num_build << "/" << num_build << ")"
            << std::endl;
#endif
  // Insertion phase
  start = timer::now();
  ...
  auto sec_insert =
      std::chrono::duration_cast<std::chrono::seconds>(end - start).count();
  std::cout << "Insertion phase in " << sec_insert << " secs. ["
            << D.size() - num_build << "]" << std::endl;
  std::cout << "Each insert takes "
  ...
  for (uint64_t i = num_build; i < D.size(); ++i) {
    if ((i - num_build) % ((D.size() - num_build) / 1000) == 0) {
      ...
      std::cout << "\r check insert: " << per << "% (" << i - num_build << "/"
                << D.size() - num_build << ")" << std::flush;
    }
  ...
  std::cout << "\r check insert: 100% (" << D.size() - num_build << "/"
            << D.size() - num_build << ")" << std::endl;
#endif
  ...
  auto sec_remove =
      std::chrono::duration_cast<std::chrono::seconds>(end - start).count();
  std::cout << "Remove phase in " << sec_remove << " secs. [" << num_build
            << "]" << std::endl;
  std::cout << "Each remove takes "
            << std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
                       .count() /
                   num_build
            << " nanosecs." << std::endl;
  ...
  for (uint64_t i = num_build; i < D.size(); ++i) {
    if ((i - num_build) % ((D.size() - num_build) / 1000) == 0) {
      float per = ((i - num_build) / (float)(D.size() - num_build)) * 100;
      std::cout << "\r check remove: " << per << "% (" << i - num_build << "/"
                << D.size() - num_build << ")" << std::flush;
    }
  ...
  std::cout << "\r check prev remove: 100% (" << D.size() - num_build << "/"
            << D.size() - num_build << ")" << std::endl;
#endif
  ...
  /* //Insertion phase
   ...
   auto sec_remove =
std::chrono::duration_cast<std::chrono::seconds>(end-start).count(); std::cout
<< "Remove phase in " << sec_remove << " secs. [" << num_build << "]" <<
std::endl; std::cout << "Each remove takes " <<
std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() /
num_build << " nanosecs." << std::endl;
   //std::cout << "Index uses " << sdsl::size_in_bytes(index) << " bytes." <<
...
")"  << std::endl; exit(0);
   ...
   std::cout << "\r check remove: 100% (" << num_build << "/" <<num_build << ")"
<< std::endl; #endif*/

  if (type == "normal") {
    query<cltj::compact_ltj_metatrie_dyn, ltj::util::trait_distinct>(
        index, queries, limit
    );
  } else if (type == "star") {
    query<cltj::compact_ltj_metatrie_dyn, ltj::util::trait_size>(
        index, queries, limit
    );
  } else {
