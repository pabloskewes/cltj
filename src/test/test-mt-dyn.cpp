...
void test(
  ...
  for (const auto &u : updates) {
    ...
    if (u.insert) {
      graph.insert(u.triple);
      if (!graph.check()) {
        std::cout << "Check false after inserting D[" << i_up << "]=("
                  << u.triple[0] << ", " << u.triple[1] << ", " << u.triple[2]
                  << ")" << std::endl;
        exit(0);
      }
      /*if(graph.test_exists(u.triple) == 0) {
          std::cout << "Error inserting D[" << i_up << "]=(" << u.triple[0] <<
      ", " << u.triple[1] << ", " << u.triple[2] << ") appears in "
                    << "0 tries." << std::endl;
          exit(0);
      }*/
      ...
      graph.remove(u.triple);
      if (!graph.check()) {
        std::cout << "Check false after removing D[" << i_up << "]=("
                  << u.triple[0] << ", " << u.triple[1] << ", " << u.triple[2]
                  << ")" << std::endl;
        exit(0);
      }
      /* if(graph.test_exists(u.triple) > 0) {
           std::cout << "Error removing D[" << i_up << "]=(" << u.triple[0] <<
       ", " << u.triple[1] << ", " << u.triple[2] << ") appears in "
                     << "0 tries." << std::endl;
           exit(0);
       }*/
