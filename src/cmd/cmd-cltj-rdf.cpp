...
template<class Index>
void interactive(const std::string &index_name, uint64_t limit, uint64_t timeout, bool print) {
    typedef typename Index::tuple_type tuple_type;
    Index m_index;
    if(::util::file::file_exists(index_name)) {
        std::cout << BLUE << "Loading index from " << index_name << "." << std::endl;
        sdsl::load_from_file(m_index, index_name);
        std::cout << "The index is loaded: " << sdsl::size_in_bytes(m_index) << " bytes."  << RESET << std::endl << std::endl;
    }else {
        std::cout << BLUE << "File " << index_name << " does not exist." << std::endl;
            ...
            if(in.size() < 2) {
                ...
                if(op == "insert"){
                    ...
                    for(auto i = 0; i < cnt; ++i) {
                        std::cout << GREEN << "[CLTJ]> " << BLUE << "[" << i+1 << "/" << cnt << "] " << RESET << std::flush;
                        std::getline(std::cin, line);
                        to_run.push_back(line);
                    }
                    std::cout << "        " << RED <<"[" << cnt << "/" << cnt << "] " << RESET << std::flush;
                    uint64_t ins = 0;
                    ...
                    std::cout << ins << " triples inserted in " << time << " ns." << std::endl;
                }else if(op == "delete"){
                    std::vector<std::string> to_run;
                    for(auto i = 0; i < cnt; ++i) {
                        std::cout << GREEN << "[CLTJ]> " << BLUE << "[" << i+1 << "/" << cnt << "] " << RESET << std::flush;
                    ...
                    auto stop = std::chrono::high_resolution_clock::now();
                    auto time = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start).count();
                    std::cout << dels << " triples deleted in " << time << " ns." << std::endl;
                }else if (op == "query"){
                    std::vector<std::string> to_run;
                    for(auto i = 0; i < cnt; ++i) {
                        std::cout << GREEN << "[CLTJ]> " << BLUE << "[" << i+1 << "/" << cnt << "] " << RESET << std::flush;
                    ...
                    if(print) {
                        ...
                        for(auto i = 0; i < cnt; ++i) {
                            ...
                            auto stop = std::chrono::high_resolution_clock::now();
                            auto time = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start).count();
                            std::cout << "        " << RED <<"[" << i+1 << "/" << cnt << "] end" << RESET << std::endl;
                            std::cout << "        " << RED <<"[" << i+1 << "/" << cnt << "] " << RESET << res.size() << " results in " << time << " ns." << std::endl;
                            res.clear();
                        }
                    }else {
                        ::util::results_collector<tuple_type> res;
                        for(auto i = 0; i < cnt; ++i) {
                            std::cout << "        " << RED <<"[" << i+1 << "/" << cnt << "] " << RESET << std::flush;
                            auto start = std::chrono::high_resolution_clock::now();
                            m_index.query(to_run[i], res, limit, timeout);
                            auto stop = std::chrono::high_resolution_clock::now();
                            auto time = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start).count();
...
int main(int argc, char **argv) {
    ...
    if(exec == "build") {
        ...
        if(args.tries == "partial") {
            print_logo();
            print(args);
            build<cltj::xcltj_rdf_dyn>(args.file, args.index+ ".xcltj");
        ...
        if(ext == ".xcltj") {
            ...
            if(args.veo == "adaptive") {
                interactive<cltj::xcltj_rdf_dyn>(args.index, args.limit, args.timeout, args.print);
            }else if(args.veo == "global") {
                interactive<cltj::xcltj_rdf_dyn_global>(args.index, args.limit, args.timeout, args.print);
            }else {
