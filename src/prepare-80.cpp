...
int main(int argc, char **argv){
    ...
    uint64_t updates = 1500000; //1295*1000 (creo mas por si en el futuro se necesitan)
    for(uint64_t i = 0; i < updates; ++i) {
        bool ins = dist1(rd);
        if(ins) {
            of_updates << "Insert" << std::endl;
            of_updates << D[i_ins][0] << " " << D[i_ins][1] << " " << D[i_ins][2] << std::endl;
            ++i_ins;
            if(i_ins == D.size()) i_ins = 0;
        }else {
            of_updates << "Delete" << std::endl;
            of_updates << D[i_del][0] << " " << D[i_del][1] << " " << D[i_del][2] << std::endl;
            ++i_del;
