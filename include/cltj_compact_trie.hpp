    ...
    public:
        ...
        compact_trie(const Trie* trie, const uint64_t n_nodes){
            ...
            while(!q.empty()){
                ...
                pos_bv = pos_bv + node->children.size() +1; //TODO: por ahora +1, despois quitar esto e ollo cos calculos
                m_bv[pos_bv]=0;
            }
        ...
        void swap(compact_trie &o) {
            ...
            */
            ...
            //**** This function only handles depth 1. ****
             uint64_t subtree_size(uint64_t it){
            ...
            pair<uint32_t, uint32_t> binary_search_seek(uint32_t val, uint32_t i, uint32_t f){
                ...
                while(i<f){
                    mid = (i + f)/2;
                    if(m_seq[mid]<val)i = mid+1;
                    else if(m_seq[mid]>=val)f = mid;
                }
