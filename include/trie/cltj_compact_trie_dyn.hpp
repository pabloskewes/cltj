...
template <uint default_width = 32> class compact_trie_dyn {
...
public:
  ...
  */
  ...
  next(size_type i, size_type j, value_type val) {
  ...
  binary_search(uint32_t val, uint32_t i, uint32_t f) {
    if (m_seq[f] < val)
      return {f + 1, false};
    uint32_t mid;
    while (i < f) {
