#ifndef CLTJ_COMPACT_TRIE_STATS_H
#define CLTJ_COMPACT_TRIE_STATS_H
...
private:
  ...
  void copy(const compact_trie_stats &o) {
    ...
    m_count_select = o.m_count_select;
  }
...
public:
  ...
  const size_type &count_select = m_count_select;

  compact_trie_stats() = default;
  ...
  compact_trie_stats(
    ...
    // std::cout << "n_nodes=" << syms.size() << " pos_seq=" << pos_seq << "
    // pos_bv=" << pos_bv << std::endl;

    sdsl::util::bit_compress(m_seq);
    // std::cout << "width of trie seq = " << (uint64_t) m_seq.width() <<
    // std::endl;
  ...
  //! Copy constructor
  compact_trie_stats(const compact_trie_stats &o) {
    copy(o);
  }

  //! Move constructor
  compact_trie_stats(compact_trie_stats &&o) {
    *this = std::move(o);
  }

  //! Copy Operator=
  compact_trie_stats &operator=(const compact_trie_stats &o) {
    if (this != &o) {
      copy(o);
    }
    return *this;
  ...
  //! Move Operator=
  compact_trie_stats &operator=(compact_trie_stats &&o) {
    if (this != &o) {
  ...
  void swap(compact_trie_stats &o) {
    ...
    std::swap(m_count_select, o.m_count_select);
  }
