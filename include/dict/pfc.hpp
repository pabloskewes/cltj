...
public:
  ...
  PFC(std::string &initial_string, uint64_t initial_size)
      : text_string(initial_string), current_size(initial_size) {
  }
  ...
  void insert(const std::string &s, uint64_t id) {
    ...
    if (prev == "\0" && comp < 0) {
      ...
      r += s + '\0';
      uint64_t lcp =
          longest_common_prefix(s, curr, std::min(s.size(), curr.size()));
      text_string.erase(0, index);
      std::string p =
          encode_number(curr_id) + encode_number(lcp) + curr.substr(lcp) + '\0';
      ...
      uint64_t lcp =
          longest_common_prefix(s, curr, std::min(s.size(), curr.size()));
      text_string +=
          encode_number(id) + encode_number(lcp) + s.substr(lcp) + '\0';
    } else {
      // Insert and update the next one
      uint64_t lcp_prev =
          longest_common_prefix(prev, s, std::min(s.size(), prev.size()));
      uint64_t lcp_after =
          longest_common_prefix(s, curr, std::min(s.size(), curr.size()));
      text_string.erase(curr_index, index - curr_index);
      std::string r = encode_number(id) + encode_number(lcp_prev) +
                      s.substr(lcp_prev) + '\0';
      std::string p = encode_number(curr_id) + encode_number(lcp_after) +
                      curr.substr(lcp_after) + '\0';
  ...
  void append(const std::string &s, uint64_t id, std::string &prev) {
    ...
    uint64_t lcp =
        longest_common_prefix(s, prev, std::min(s.size(), prev.size()));
    text_string +=
        encode_number(id) + encode_number(lcp) + s.substr(lcp) + '\0';
    current_size++;
  }
   ...
   * @return bool, uint64_t A flag that indicates if s exists and its ID, in
   * that case
   */
  ...
  std::string extract(uint64_t i) {
    ...
    throw std::invalid_argument(
        "ID is not asociated to any string in Plain Front Coding"
    );
  }
  ...
  uint64_t elim(const std::string &s) {
    ...
    if (index >= text_string.size()) {
      ...
      uint64_t lcp = decode_number(index);
      std::string next = encode_number(next_id) + curr.substr(0, lcp) +
                         read_string(index) + '\0';
      text_string.erase(0, index);
      ...
      std::string p = encode_number(next_id) + encode_number(lcp_with_prev) +
                      next.substr(lcp_with_prev) + '\0';
      text_string.erase(curr_index, index - curr_index);
  ...
  void elim(const uint64_t &id) {
    ...
    if (index >= text_string.size()) {
      ...
      uint64_t lcp_with_prev =
          longest_common_prefix(prev, next, std::min(prev.size(), next.size()));
      std::string p = encode_number(next_id) + encode_number(lcp_with_prev) +
                      next.substr(lcp_with_prev) + '\0';
      text_string.erase(curr_index, index - curr_index);
      text_string.insert(curr_index, p);
   ...
   *  (The string corresponding to the second half, The size of the second half,
   * The first word of the second half)
   */
  std::tuple<std::string, uint64_t> split() {
    ...
    std::tuple<std::string, uint64_t> response =
        std::make_tuple(second_half, current_size - middle);
  ...
  void fuse(std::string &new_half, uint64_t new_words_counter) {
    ...
    uint64_t lcp = longest_common_prefix(
        curr, first_word, std::min(curr.size(), first_word.size())
    );
    new_half.erase(0, index);
    new_half.insert(
        0, encode_number(new_id) + encode_number(lcp) + first_word.substr(lcp) +
               '\0'
    );
...
private:
  ...
  void read_string(
    ...
    uint64_t new_index = text_string.find_first_of('\0', index);
    container =
        prev.substr(0, lcp) + text_string.substr(index, new_index - index);
    index = new_index + 1;
  }
  ...
  /**
   * @brief Calculates the length of the longest common prefix between two
   * strings
   *
   ...
   */
  uint64_t longest_common_prefix(
      const std::string &s1,
      const std::string &s2,
      size_t max_length
  ) {
