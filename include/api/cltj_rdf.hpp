...
template <
    class Index = cltj::compact_dyn_ltj,
...
private:
  ...
  void build(const std::string &dataset) {
    ...
    std::cout << "Ratio    : " << (size_index) / (double)size_data * 100 << "%"
              << std::endl;
    std::cout << "============================================================"
              << std::endl;
  }
...
public:
  ...
  bool query(
      ...
      result_type &res,
      size_type limit = 1000,
      size_type timeout_seconds = 600
  ) {
    ...
    std::vector<cltj::user_triple> tokens =
        ::util::rdf::str::get_query(query_str);
    for (cltj::user_triple &token : tokens) {
      auto p = ::util::rdf::str::get_triple_pattern(
          token, ht_var_id, var_in_p, m_dict_so, m_dict_p
      );
      if (!p.first)
        return false;
  ...
  bool remove(const std::string &triple) {
    ...
    if (r.removed) {
      if (r.rem_in_dict[0])
        m_dict_so.eliminate(spo[0]);
      if (r.rem_in_dict[1])
        m_dict_p.eliminate(spo[1]);
      if (r.rem_in_dict[2])
        m_dict_so.eliminate(spo[2]);
      return true;
    }
    return false;
  }
...
typedef cltj::cltj_rdf<
    cltj::compact_dyn_ltj,
    ltj::util::trait_size,
...
typedef cltj::cltj_rdf<
    cltj::compact_ltj_metatrie_dyn,
    ltj::util::trait_size,
    ltj::ltj_iterator_metatrie<
        cltj::compact_ltj_metatrie_dyn,
        uint8_t,
        uint64_t>>
    xcltj_rdf_dyn;
...
typedef cltj::cltj_rdf<
    ...
    ltj::ltj_iterator_metatrie<
        ...
        uint8_t,
        uint64_t>,
    ltj::veo::veo_simple<ltj::ltj_iterator_metatrie<
        cltj::compact_ltj_metatrie_dyn,
        uint8_t,
        uint64_t>>>
    xcltj_rdf_dyn_global;
