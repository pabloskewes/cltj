...
void copyBits (uint64_t *tgt, uint64_t ptgt,
   ...
   { uint64_t old,mask;
     ...
     tgt += ptgt/w64; ptgt %= w64;
     src += psrc/w64; psrc %= w64;
     mask = (((uint64_t)1) << ptgt)-1;
     if (ptgt == psrc)
        { if (ptgt != 0)
             { *tgt = (*tgt & mask) + (*src & ~mask);
               *tgt++; *src++; len -= w64-ptgt;
             }
     ...
     if (ptgt < psrc) // first word from src fits in ptgt
        ...
        { *tgt = (*tgt & mask) + ((*src << (ptgt-psrc)) & ~mask);
          if (len <= w64-ptgt) return; // before overflowing to the next word
          ptgt -= psrc;
          *++tgt = *src++ >> (w64-ptgt);
        }
     if (len <= w64-psrc) return;
     len -= w64-psrc;
