...
leafBVId leafBVIdCreateFrom64(
  ...
  // sequence
  B->width = width;
  B->id_data = (uint64_t *)mycalloc(
      sequenceSize(width) / sizeof(uint64_t), sizeof(uint64_t)
  );
  p = 0;
  for (i = 0; i < n; i++) {
    word = id_data[i];
...

{
  ...
  B->ones = 0;
  for (uint k = 0; k < nb; k++)
    B->ones += popcount(B->data[k]);

  // sequence
  B->id_data = (uint64_t *)myalloc(sequenceSize(width));
  if (i == 0)
    memcpy(B->id_data, id_data, (n * width + 7) / 8);
  else
    copyBits(B->id_data, 0, id_data, i * width, n * width);
  if (freeit)
    myfree(id_data);
  return B;
}
...
void leafBVIdInsertId(leafBVId B, uint i, uint64_t v) {
  ...
  if (B->width == w64) {
    for (b = nb; b > ib; b--)
      B->id_data[b] = B->id_data[b - 1];
    B->id_data[ib] = v;
    return;
  }
  for (b = nb; b > ib; b--)
    B->id_data[b] =
        (B->id_data[b] << B->width) | (B->id_data[b - 1] >> (w64 - B->width));
  if (ir + B->width < w64)
    B->id_data[ib] =
        (B->id_data[ib] & ((((uint64_t)1) << ir) - 1)) | (v << ir) |
        ((B->id_data[ib] << B->width) & ((~(uint64_t)0) << (ir + B->width)));
  else {
    B->id_data[ib] = (B->id_data[ib] & ((((uint64_t)1) << ir) - 1)) | (v << ir);
    B->id_data[ib + 1] =
        (B->id_data[ib + 1] & ((~(uint64_t)0) << (ir + B->width - w64))) |
        (v >> (w64 - ir));
  }
}
...
void leafBVIdInsert(leafBVId B, uint i, uint bv_v, uint64_t v, uint first) {
  ...
  nb = (B->size + 1) / w64;
  if (!bv_v && first)
    ++i; // append the 0-bit after the first child (current position i)
  ib = i / w64;
  for (b = nb; b > ib; b--)
    B->data[b] = (B->data[b] << 1) | (B->data[b - 1] >> (w64 - 1));
  if ((i + 1) % w64) {
    B->data[ib] = (B->data[ib] & ((((uint64_t)1) << (i % w64)) - 1)) |
                  (((uint64_t)bv_v) << (i % w64)) |
                  ((B->data[ib] << 1) & (~((uint64_t)0) << ((i + 1) % w64)));
    // if(!bv_v && first) B->data[ib] ^= (((uint64_t) 3) << (i % w64)); //xor to
    // interchange 0-bit and 1-bit
  } else {
...
int leafBVIdDelete(leafBVId B, uint i, uint more, uint *nl) {
  ...
  *nl = 0;
  if (v && more) { // when it removes a 1-bit but it has more siblings
    *nl =
        (B->size - 1 == i
        ); // update next leaf with a 1-bit at the first postion
    if (!*nl) {
      B->data[ib] |= ((uint64_t)1)
                     << (i % w64); // update current leaf with the removed 1-bit
      v = 0;
    }
  }
  B->ones -= v;
