...
hybridBVId hybridBVIdCreateFrom64(
    uint64_t *bv_data,
    uint64_t *id_data,
    uint64_t n,
    uint width
...
hybridBVId hybridBVIdCreateFromPacked(
    uint64_t *bv_data,
    uint64_t *id_data,
    uint64_t n,
    uint width
...
hybridBVId hybridBVIdClone(hybridBVId B) {
  ...
  BC->type = B->type;
  if (B->type == tLeaf)
    BC->bv.leaf = leafBVIdClone(B->bv.leaf);
  else if (B->type == tStatic)
    BC->bv.stat = staticBVIdClone(B->bv.stat);
  else {
...
void hybridBVIdDestroy(hybridBVId B) {
  if (B->type == tLeaf)
    leafBVIdDestroy(B->bv.leaf);
  else if (B->type == tStatic)
    staticBVIdDestroy(B->bv.stat);
  else {
...
/*static void packedReadoold (hybridBVId B, uint64_t *bv_D, uint64_t *id_D,
uint64_t j)
...
{ uint64_t lsize;
    ...
    if (B->type == tStatic) {
        ...
        copyBits(id_D, j*width, B->bv.stat->id_data, 0,
hybridBVIdLength(B)*width); }else {
        copyBits(bv_D,j,B->bv.leaf->data,0,hybridBVIdLength(B));
        copyBits(id_D, j*width, B->bv.leaf->id_data, 0,
hybridBVIdLength(B)*width);
    }
}*/
...
static void packedRead(hybridBVId B, uint64_t *D, uint64_t j) {
  ...
  if (B->type == tStatic)
    ...
    copyBits(D, j * width, B->bv.leaf->id_data, 0, hybridBVIdLength(B) * width);
}

static void
myread(hybridBVId B, uint64_t i, uint64_t l, uint64_t *D, uint64_t j) {
  uint64_t lsize;
  if (B->type == tLeaf) {
  ...
  if (B->type == tDynamic) {
    lsize = hybridBVIdLength(B->bv.dyn->left);
    if (i + l < lsize)
      myread(B->bv.dyn->left, i, l, D, j);
    else if (i >= lsize)
      myread(B->bv.dyn->right, i - lsize, l, D, j);
    else {
...
static void collect(
  ...
  *id_D =
      (uint64_t *)myalloc(((len * width + w64 - 1) / w64) * sizeof(uint64_t));
  packedRead(B, *id_D, 0);
...
static dynamicBVId splitFrom(
  ...
  while (nblock >= 2) {
    ...
    if (i / bnum < nblock / 2) {
      ...
      if (n - (nblock / 2) * bnum > leafBVIdNewSize(width)) {
        // create a static
        segment =
            (uint64_t *)myalloc((end - mid + w64 - 1) / w64 * sizeof(uint64_t));
        memcpy(
            segment, mid_ptr, (end - mid + w64 - 1) / w64 * sizeof(uint64_t)
        );

        id_segment = (uint64_t *)myalloc(
            (((end - mid) * width + w64 - 1) / w64) * sizeof(uint64_t)
        );
        copyBits(id_segment, 0, id_data, mid * width, (end - mid) * width);
        HB->type = tStatic;
        HB->bv.stat = staticBVIdCreateFromPacked(
            segment, id_segment, n - (nblock / 2) * bnum, width
        );
      } else {
        // create a leaf
        HB->type = tLeaf;
        HB->bv.leaf = leafBVIdCreateFromPacked(
            (uint64_t *)mid_ptr, id_data, mid, n - (nblock / 2) * bnum, width, 0
        );
      }
      ...
      if ((nblock / 2) * bnum > leafBVIdNewSize(width)) {
        ...
        segment = (uint64_t *)myalloc(
            (mid - start + w64 - 1) / w64 * sizeof(uint64_t)
        );
        memcpy(
            segment, start_ptr, (mid - start + w64 - 1) / w64 * sizeof(uint64_t)
        );
        id_segment = (uint64_t *)myalloc(
        ...
        HB->type = tStatic;
        HB->bv.stat = staticBVIdCreateFromPacked(
            segment, id_segment, (nblock / 2) * bnum, width
        );
      } else {
        // create a leaf
        HB->type = tLeaf;
        HB->bv.leaf = leafBVIdCreateFromPacked(
            (uint64_t *)start_ptr, id_data, start, (nblock / 2) * bnum, width, 0
        );
      }
  ...
  HB->type = tLeaf;
  HB->bv.leaf = leafBVIdCreateFromPacked(
      (uint64_t *)start_ptr, id_data, start, n, width, 0
  );
  return finalDB;
}
...
static dynamicBVId split(staticBVId B, uint64_t i) {
  ...
  dynamicBVId DB;
  DB = splitFrom(
      B->data, B->id_data, staticBVIdLength(B), staticBVIdOnes(B), B->width, i
  );
  staticBVIdDestroy(B);
  return DB;
}
...
static dynamicBVId splitLeaf(leafBVId B) {
  ...
  HB1->type = tLeaf;
  HB1->bv.leaf = LB;
  LB = leafBVIdCreateFromPacked(
      (uint64_t *)(((byte *)B->data) + bsize), B->id_data, bnum, B->size - bnum,
      B->width, 0
  );
...
static leafBVId mergeLeaves(dynamicBVId B) {
  ...
  copyBits(
      LB1->id_data, LB1->size * B->width, LB2->id_data, 0, LB2->size * B->width
  );
  LB1->size += LB2->size;
  LB1->ones += LB2->ones;
  leafBVIdDestroy(LB2);
...
static int transferLeft(dynamicBVId B) {
  ...
  if (trf < leafBVIdMaxSize(B->width) * TrfFactor)
    return 0;
  // bitvector
  ...
  if (trf % w64)
    ones += popcount(LB2->data[words] & (((uint64_t)1) << (trf % w64)) - 1);
  LB1->ones += ones;
  LB2->ones -= ones;
...
static int transferRight(dynamicBVId B) {
  ...
  copyBits(
      LB2->id_data, 0, LB1->id_data, (LB1->size - trf) * B->width,
      trf * B->width
  );
...
void hybridBVIdSave(hybridBVId B, FILE *file) {
  ...
  if (B->type == tLeaf)
    leafBVIdSave(B->bv.leaf, file);
  else
    staticBVIdSave(B->bv.stat, file);
}
...
uint64_t hybridBVIdSpace(hybridBVId B) {
  ...
  if (B->type == tLeaf)
    return s + leafBVIdSpace(B->bv.leaf);
  else if (B->type == tStatic)
    return s + staticBVIdSpace(B->bv.stat);
...
extern inline uint64_t hybridBVIdLength(hybridBVId B) {
  if (B->type == tLeaf)
    return leafBVIdLength(B->bv.leaf);
  if (B->type == tStatic)
    return staticBVIdLength(B->bv.stat);
...
extern inline uint64_t hybridBVIdLast(hybridBVId B) {
  ...
  if (B->type == tStatic)
    return staticBVIdAccessId(B->bv.stat, staticBVIdLength(B->bv.stat) - 1);
...
extern inline uint64_t hybridBVIdOnes(hybridBVId B) {
  if (B->type == tLeaf)
    return leafBVIdOnes(B->bv.leaf);
  if (B->type == tStatic)
    return staticBVIdOnes(B->bv.stat);
...
int hybridBVIdWriteBV(hybridBVId B, uint64_t i, uint64_t v) {
  ...
  lsize = hybridBVIdLength(B->bv.dyn->left);
  if (i < lsize)
    dif = hybridBVIdWriteBV(B->bv.dyn->left, i, v);
  else
    dif = hybridBVIdWriteBV(B->bv.dyn->right, i - lsize, v);
  B->bv.dyn->ones += dif;
  return dif;
}
...
void hybridBVIdSplit(hybridBVId B, uint64_t i) {
  ...
  if (B->type == tLeaf) {
    if (leafBVIdLength(B->bv.leaf) ==
        leafBVIdMaxSize(B->bv.leaf->width)) // split
    {
      B->type = tDynamic;
  ...
  lsize = hybridBVIdLength(B->bv.dyn->left);
  if (i < lsize)
    hybridBVIdSplit(B->bv.dyn->left, i);
  else
    hybridBVIdSplit(B->bv.dyn->right, i - lsize);
}
...
static void irecompute(hybridBVId B, uint64_t i) {
  ...
  if (B->type == tDynamic) {
    ...
    B->bv.dyn->leaves =
        hybridBVIdLeaves(B->bv.dyn->left) + hybridBVIdLeaves(B->bv.dyn->right);
  }
}
...
static int64_t insert(
  ...
  if (i < lsize) {
    if ((lsize == leafBVIdMaxSize(width))   // will overflow if leaf
        ...
        && (B->bv.dyn->right->type == tLeaf) && transferRight(B->bv.dyn)) {
    ...
    if ((rsize == leafBVIdMaxSize(width))    // will overflow if leaf
        ...
        && (B->bv.dyn->left->type == tLeaf) && transferLeft(B->bv.dyn)) {
...
int hybridBVIdDelete(hybridBVId B, uint64_t i, uint more) {
  ...
  if (nl)
    hybridBVIdWriteBV(B, i, 1); // restoring a 1-bit
  return r.diff;
}
...
static uint64_t rank(hybridBVId B, uint64_t i, int64_t *delta) {
  ...
  if (B->type == tDynamic) {
    ...
    if (mustFlatten(B))
      ...
      lsize = hybridBVIdLength(B->bv.dyn->left);
      if (i < lsize)
        return rank(B->bv.dyn->left, i, delta);
      else
  ...
  if (B->type == tLeaf)
    return leafBVIdRank(B->bv.leaf, i);
...
static int64_t next1(hybridBVId B, uint64_t i, int64_t *delta) {
  ...
  if (B->type == tLeaf)
    return leafBVIdNext1(B->bv.leaf, i);
  ...
  if (B->type == tDynamic) {
    ...
    if (mustFlatten(B)) {
      ...
      if (i >= lsize)
        return lsize +
               next(
                   B->bv.dyn->right, i - lsize, j - lsize, c, recomp, value, all
               );
      n = next(B->bv.dyn->left, i, lsize - 1, c, recomp, value, 1);
      if (n < lsize)
        return n;
      return lsize +
             next(B->bv.dyn->right, 0, j - lsize, c, recomp, value, all);
    }
  }
  if (B->type == tLeaf)
    return leafBVIdNext(B->bv.leaf, i, j, c, value);
  else
    return staticBVIdNext(B->bv.stat, i, j, c, value);
}

uint64_t hybridBVIdNext(
    hybridBVId B,
    uint64_t i,
    uint64_t j,
    uint64_t c,
...
uint checkOnes(hybridBVId B) {
  if (B->type == tDynamic) {
    uint64_t ones =
        hybridBVIdOnes(B->bv.dyn->left) + hybridBVIdOnes(B->bv.dyn->right);
    return ones == hybridBVIdOnes(B) && checkOnes(B->bv.dyn->left) &&
           checkOnes(B->bv.dyn->right);
  }
  if (B->type == tLeaf) {
...
uint checkLast(hybridBVId B) {
  if (B->type == tDynamic) {
    return checkLast(B->bv.dyn->left) &&
           (hybridBVIdLast(B->bv.dyn->right) == hybridBVIdLast(B));
  }
  return 1;
}
...
uint checkLastPrint(hybridBVId B) {
  if (B->type == tDynamic) {
    ...
    if (hybridBVIdLast(B->bv.dyn->right) != hybridBVIdLast(B)) {
      printf(
          "falla en dynamic con: %lu, %lu\n", hybridBVIdLast(B),
          hybridBVIdLength(B)
      );
      if (B->bv.dyn->left->type == tStatic) {
        printf(
            "-left: static %lu, %lu \n", hybridBVIdLast(B->bv.dyn->left),
            hybridBVIdLength(B->bv.dyn->left)
        );
      ...
      if (B->bv.dyn->right->type == tStatic) {
        printf(
            "-right: static %lu, %lu \n", hybridBVIdLast(B->bv.dyn->right),
            hybridBVIdLength(B->bv.dyn->right)
        );
...
uint checkOnesPrint(hybridBVId B) {
  if (B->type == tDynamic) {
    ...
    if (ones == hybridBVIdOnes(B)) {
      return checkOnesPrint(B->bv.dyn->left) &&
             checkOnesPrint(B->bv.dyn->right);
    }
