...
void hybridDestroy(hybridBV B) {
  if (B->type == tLeaf)
    leafDestroy(B->bv.leaf);
  else if (B->type == tStatic)
    staticDestroy(B->bv.stat);
  else {
...
static void
myread(hybridBV B, uint64_t i, uint64_t l, uint64_t *D, uint64_t j) {
  uint64_t lsize;
  if (B->type == tLeaf) {
...
static dynamicBV splitLeaf(leafBV B) {
  ...
  HB1->bv.leaf = LB;
  LB = leafCreateFrom(
      (uint64_t *)(((byte *)B->data) + bsize), B->size - bsize * 8, 0
  );
...
static dynamicBV
splitFrom(uint64_t *data, uint64_t n, uint64_t ones, uint64_t i) {
  hybridBV HB;
  ...
  while (nblock >= 2) {
    ...
    if (i < (nblock / 2) * blen) {
      ...
      if (n - (nblock / 2) * blen > leafNewSize() * w64) {
        ...
        HB->type = tLeaf;
        HB->bv.leaf =
            leafCreateFrom((uint64_t *)mid, n - (nblock / 2) * blen, 0);
      }
...
static int transferLeft(dynamicBV B) {
  ...
  if (trf < leafMaxSize() * w64 * TrfFactor)
    return 0;
...
uint64_t hybridSpace(hybridBV B) {
  ...
  if (B->type == tLeaf)
    return s + leafSpace(B->bv.leaf);
  else if (B->type == tStatic)
    return s + staticSpace(B->bv.stat);
...
extern inline uint64_t hybridLength(hybridBV B) {
  if (B->type == tLeaf)
    return leafLength(B->bv.leaf);
  if (B->type == tStatic)
    return staticLength(B->bv.stat);
...
extern inline uint64_t hybridOnes(hybridBV B) {
  if (B->type == tLeaf)
    return leafOnes(B->bv.leaf);
  if (B->type == tStatic)
    return staticOnes(B->bv.stat);
...
int hybridWrite(hybridBV B, uint64_t i, uint v) {
  ...
  lsize = hybridLength(B->bv.dyn->left);
  if (i < lsize)
    dif = hybridWrite(B->bv.dyn->left, i, v);
  else
    dif = hybridWrite(B->bv.dyn->right, i - lsize, v);
  B->bv.dyn->ones += dif;
  return dif;
}
...
static void irecompute(hybridBV B, uint64_t i) {
  ...
  if (B->type == tDynamic) {
    ...
    B->bv.dyn->leaves =
        hybridLeaves(B->bv.dyn->left) + hybridLeaves(B->bv.dyn->right);
  }
}
...
static void insert(hybridBV B, uint64_t i, uint v, uint *recalc) {
  ...
  if (i < lsize) {
    ...
    if ((rsize == leafMaxSize() * w64)      // will overflow if leaf
        ...
        && (B->bv.dyn->left->type == tLeaf) // both are leaves
        && (B->bv.dyn->right->type == tLeaf) && transferLeft(B->bv.dyn)) {
      // avoided, transferred to right
...
static void sread(
  ...
  if (B->type == tDynamic) {
    ...
    if (mustFlatten(B)) {
      ...
      lsize = hybridLength(B->bv.dyn->left);
      if (i + l < lsize)
        sread(B->bv.dyn->left, i, l, D, j, recomp);
      else if (i >= lsize)
        sread(B->bv.dyn->right, i - lsize, l, D, j, recomp);
      else {
