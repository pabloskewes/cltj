...
{ uint i,p;
	...
	B->isStat = isStat;
	if (isStat)
	{ if (B->width == w64) { B->data = data; return B; }
		B->data = (uint64_t*)mycalloc(((n*width+w64-1)/w64),sizeof(uint64_t));
	}
	...
	{ word = data[i];
		...
		if ((p%w64)+width > w64) B->data[p/w64+1] = word >> (w64-(p%w64));
		p += width;
	}
	return B;
}
...
void leafIdSave (leafId B, FILE *file)
   ...
   { myfwrite(&B->size,sizeof(uint64_t),1,file);
     ...
     if (B->size != 0)
        myfwrite (B->data,sizeof(uint64_t),(B->size*B->width+w64-1)/w64,file);
   }
...
leafId leafIdLoad (FILE *file)
    ...
    { leafId B;
      ...
      data = (uint64_t*)myalloc(((size*width+w64-1)/w64)*sizeof(uint64_t));
...
void leafIdWrite (leafId B, uint i, uint64_t v)
   ...
   { uint64_t word;
     uint iq,ir;
     if (B->width == w64) { B->data[i] = v; return; }
...
void leafIdInsert (leafId B, uint i, uint64_t v)
   ...
   { uint nb = ++B->size*B->width/w64;
	...
	B->data[ib] = (B->data[ib] & ((((uint64_t)1) << ir) - 1)) | (v << ir);
	B->data[ib+1] = (B->data[ib+1] & ((~(uint64_t)0) << (ir+B->width-w64))) |
			(v >> (w64-ir));
        }
   }
...
extern inline uint64_t leafIdAccess (leafId B, uint i)
   ...
   { uint64_t word;
     if (B->width == w64) return B->data[i];
     uint iq = i*B->width/w64;
     uint ir = (i*B->width)%w64;
     word = (B->data[iq] >> ir);
     if (ir+B->width > w64) word |= B->data[iq+1] << (w64-ir);
